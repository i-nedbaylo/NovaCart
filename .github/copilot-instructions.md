...
- Copilot **обязан** писать ответы в чате на **русском языке**.
...
NovaCart — **учебно-демонстрационный проект**. Некоторые решения намеренно упрощены для фокуса на архитектурных паттернах, а не на production-hardening. Если в коде присутствует упрощение, связанное с учебным характером проекта (например, hardcoded connection string в design-time фабрике, отсутствие rate limiting и т.д.), **обязательно** документировать это:

- **В коде:** добавить комментарий `// NOTE: Simplified for demo purposes. In production, ...` с объяснением, что следует изменить в production.
- **В документации:** при наличии системных упрощений (например, отсутствие Kubernetes на ранних фазах) — описать в `docs/architecture/decisions/` или в `README.md`.

Это предотвращает вопросы при код-ревью о том, почему реализация не соответствует production-стандартам.
...
2. **Коммит и push по каждому дробному пункту чек-листа:** после завершения каждого дробного пункта (например, 2.1, 2.2, 2.3 и т.д.) — создать коммит и выполнить `git push`. Один дробный пункт = один коммит + push.
...
4. **Не коммитить сломанный код:** перед коммитом убедиться, что solution собирается (`dotnet build`) и тесты проходят.
5. **Ожидание код-ревью после push:** после выполнения `git push` — **остановить работу** и ожидать результата pull request и замечаний по итогу код-ревью. Не переходить к следующему пункту чек-листа до завершения ревью.
6. **Итерации по замечаниям:** при наличии замечаний от ревьювера — исправить все замечания, создать коммит с исправлениями (формат: `fix(scope): address PR review feedback`), выполнить `git push`, и снова ожидать результата ревью. Повторять до одобрения.
7. **Продолжение работы:** после одобрения PR (или явной команды пользователя на продолжение) — перейти к следующему пункту чек-листа. **Без команды пользователя не продолжать.**
8. **Завершение фазы:** по завершении всех задач фазы — ветка остаётся (merge в `main` выполняет пользователь вручную).
